...
@Component({
  selector: 'app-workshop-inspection',
  templateUrl: './workshop-inspection.page.html',
  styleUrls: ['./workshop-inspection.page.scss'],
})
export class WorkshopInspectionPage implements OnInit {
  ...
  async checkInspecs(truck) {
    this.afs.collection('inspections').ref.where('code', '==', truck.code).where('jobNo', '==', truck.jobNo).where('type', '==', 'workshop').limit(1).get().then(query => {
      if (query.size > 0) {
      ...
      buttons: [
        {
          ...
          handler: data => {
            this.router.navigate([`workshop-truck`, { id1: this.code, id2: this.type, id3: this.checkType, edit: true }]);
          }
        },
        {
          text: 'OK',
  ...
  truckRoute() {
    this.router.navigate([`workshop-truck`, { id1: this.code, id2: this.type, id3: this.checkType }]);
  }

  trailerRoute() {
    this.router.navigate([`workshop-trailer`, { id1: this.code, id2: this.type, id3: this.checkType }]);
  }
